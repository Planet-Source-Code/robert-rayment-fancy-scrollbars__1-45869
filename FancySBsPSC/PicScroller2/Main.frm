...
' PicScroller2 as PicScroller but INCLUDING use of right mouse
' button to go directly to max or min.  Also delay in matching
' controls to scrollbars so the effect can be seen.
...
Dim i As Long
   ...
   Me.Caption = " Pic Scroller2  by Robert Rayment"
   
   Me.Print "        Also uses right mouse button"
   ...
   Me.Show
   Me.Refresh  ' To briefly show state before aligning
...
Private Sub DrawSillyPicture()
   ...
   Dim ixmax As Long, iymax As Long
   Dim R As Long, G As Long, B As Long

   R = 128: G = 0: B = 0
   
   ixmax = Picture2.Width - 1
   iymax = Picture2.Height - 1
   
   For j = 0 To iymax Step 4
      
      Picture2.Line (j, iymax)-(ixmax, j), RGB(R, G, B)
      
      For i = 0 To ixmax Step 4
      
         Picture2.Line (0, j)-(i, 0), RGB(R, G, B)
...
Private Sub cmdHSLEF_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
   HSLEF_MouseDown HS(Index), cmdHSTUB(Index), Index, Button
End Sub
...
Private Sub cmdHSRIT_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
   HSRIT_MouseDown HS(Index), cmdHSTUB(Index), Index, Button
End Sub
...
Private Sub cmdVSTOP_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
   VSTOP_MouseDown VS(Index), cmdVSTUB(Index), Index, Button
End Sub
...
Private Sub cmdVSBOT_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
   VSBOT_MouseDown VS(Index), cmdVSTUB(Index), Index, Button
End Sub
