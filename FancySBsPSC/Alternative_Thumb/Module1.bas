...
'                  cmdHSLEF(0), picHSTUB(0), cmdHSRIT(0)
...
'                  cmdVSTOP(0), picVSTUB(0), cmdVSBOT(0)
...
Public Sub HSLEF_MouseDown(HScr As Control, thumb As PictureBox, Index As Integer)
...
Public Sub HSRIT_MouseDown _
 (HScr As Control, thumb As PictureBox, Index As Integer)
...
Public Sub HSThumb_MouseMove(HScr As Control, thumb As PictureBox, _
...
Public Sub HSPic_MouseDown(HScr As Control, thumb As PictureBox, _
...
'       Controls:  Left,           Thumb,         Right
'                  cmdHSLEF(Index), picHSTUB(Index, cmdHSRIT(INdex)
...
With frm
   ...
   ' Scale pics & command buttons to HS size
   ' Container
   ...
   ' Horz Thumb
   .picHSTUB(Index).Top = 0
   .picHSTUB(Index).Left = .cmdHSLEF(Index).Width   ' Thumb @ Left pos
   .picHSTUB(Index).Height = .picHS(Index).Height * STY
   '------------------------------------------------------
   ' Hide HS - still operative
   .HS(Index).Visible = False
   
   ' Calc Thumb's (picHSTUB(Index)'s) max & min location on picHS(Index)
   ...
   zHSMax(Index) = .cmdHSRIT(Index).Left - .picHSTUB(Index).Width
...
Public Sub VSTOP_MouseDown(VScr As Control, thumb As PictureBox, Index As Integer)
...
Public Sub VSBOT_MouseDown(VScr As Control, thumb As PictureBox, Index As Integer)
...
Public Sub VSThumb_MouseMove(VScr As Control, thumb As PictureBox, _
 Index As Integer, Button As Integer, X As Single, Y As Single)
...
Public Sub VSPic_MouseDown(VScr As Control, thumb As PictureBox, _
 Index As Integer, X As Single, Y As Single)
...
'       Controls:  Top,             Thumb,          Bottom
'                  cmdVSTOP(Index), picVSTUB(Index), cmdVSBOT(Index)
...
With frm
   ...
   ' Scale pics & command buttons to VS size
   ' Container
   ...
   ' Vert Thumb
   .picVSTUB(Index).Top = .cmdVSTOP(Index).Height   ' Thumb top pos
   .picVSTUB(Index).Left = 0
   .picVSTUB(Index).Width = .picVS(Index).Width * STY
   '------------------------------------------------------
   ' Hide VS - still operative
   .VS(Index).Visible = False
   
   ' Calc Thumb's (picVSTUB(Index)'s) max & min location on picVS(Index)
   ...
   zVSMax(Index) = .cmdVSBOT(Index).Top - .picVSTUB(Index).Height
